...
from yarl.infrastructure.logger import Logger
from yarl.infrastructure import utils, pytorch_util as ptu
...
class RL_Trainer(object):
    ...
    def __init__(self, params):
        ...
        ac_dim = self.env.action_space.n if discrete else \
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
        ...
        for itr in range(n_iter):
            ...
            if relabel_with_expert and itr >= start_relabel_with_expert:
                paths = self.do_relabel_with_expert(expert_policy,
                                                    paths)
            ...
            if self.log_video or self.log_metrics:
                ...
                if self.params['save_params']:
                    print('\nSaving agent params')
                    self.agent.save(
                        '{}/policy_itr_{}.pt'.format(self.params['logdir'],
                                                     itr))
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths,
        ...
        if self.log_metrics:
            ...
            eval_returns = [eval_path["reward"].sum() for eval_path in
                            eval_paths]

            # episode lengths, for logging
            train_ep_lens = [len(path["reward"]) for path in paths]
            eval_ep_lens = [len(eval_path["reward"]) for eval_path in
                            eval_paths]
